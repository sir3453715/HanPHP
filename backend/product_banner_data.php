...
switch ($_GET['func']):
    case "update":
        $product_banner_id = $_GET['id'];
        $strSQL = "SELECT * FROM product_banner WHERE id = '$product_banner_id'";        
        ...
        if($_POST['flag'] == 'true'){
            ...
            if($_FILES['img']['tmp_name']){
                ...
                if($ext=='png' || $ext=='gif'){
                    Func::ImageResize_Transparent($_FILES['img']['tmp_name'], __SERVER_IMAGES_FOLDER . DIRECTORY_SEPARATOR . $img, 1280, 551);
                }else{
                    Func::ImageResize($_FILES['img']['tmp_name'], __SERVER_IMAGES_FOLDER . DIRECTORY_SEPARATOR . $img, 1280, 551);
                }
            }
            if ($del_img == 'true')
            {
            ...
            $a = $db->update('product_banner',$data,"id = '$product_banner_id'");
            if(!$a){
        ...
        array_push($data, array(
                ...
                'help' => '建議寬高: 1280 * 551 ，或等比例之圖片。其檔案大小不得超過2M',
                'delete' => true
            )
        );
        array_push($data, array(
    ...
    case "delete":
        ...
        $db->delete('product_banner',"id='$product_banner_id'");
        Func::go_to($func_page . '.php');
        exit;
        break;
    ...
    case "insert":
        if($_POST['flag'] == 'true'){
            ...
            // print_r($data);
            // exit;
            $a = $db->insert('product_banner',$data);
            if($a){
    ...
    default:
        ...
        $strSQL = "SELECT * FROM product_banner WHERE 1 $andSQL ORDER BY sort desc";
        // echo $strSQL;
        // exit;

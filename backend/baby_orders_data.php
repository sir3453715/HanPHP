...
switch ($_GET['func']):
    case "update":
		...
		//更新為已讀
		$data = array('isread'=>'1','update_date'=>time(),);
        ...
        $content.="<tr class ='unsortable'><th class='text-center'>名稱</th><th class='text-center'>單價</th><th class='text-center'>數量</th><th class='text-center'>小計</th><th class='text-center'>備註</th><tr class ='unsortable'>";
        $i = 1;
        $notot = 0;
        if (is_array($arr_data2)):
            foreach ($arr_data2 as $kk => $data)
            {
                ...
                $name = $data['name'];
                $num = $data['num'];
                $strSQL = "SELECT * FROM baby_product WHERE id='".$data['item_sn']."'";
                $baby_product = $db->Execute($strSQL);
                if($baby_product[0]['discounted_price']>0){
                    $price = $baby_product[0]['discounted_price'];
                }else{
                    $price = $baby_product[0]['original_price'];
                }
                $stot = $data['price'] * $num;
                $notot += $stot;
                switch ($data['ipp']) {
        ...
        if($_POST['flag'] == 'true'){
            ...
            if($state==5 && $arr_data[0]['state']!=5){//訂單取消
                ...
                //檢查是否有發放點數
                if($arr_data[0]['red_sent']==1){//點數送過
                    //商品送的點數
                    $strSQL = "SELECT * FROM orders_item WHERE id='".$arr_data[0]['sn']."'";
                    $orders_item = $db->Execute($strSQL);
                    foreach ($orders_item as $data){
                        $strSQL = "SELECT * FROM baby_product WHERE id='".$data['item_sn']."'";
                        $baby_product = $db->Execute($strSQL);
                        // if($data['ipp']==0){
                            $total_red += $baby_product[0]['red'] * $data['num'];
                        // }
                    }
                    //把原先送的點數立即失效
                    $strSQL = "SELECT * FROM point WHERE cust_id='".$arr_data[0]['cust_id']."' AND seccode='".$arr_data[0]['seccode']."' AND type='add' ORDER BY id DESC LIMIT 1";
                    $send_point = $db->Execute($strSQL);
                    $data = array('create_date'=>time(),'use_day'=>0,'type'=>'balance');
                    $db->update('point',$data,"id = '".$send_point[0]['id']."'");

                    //扣回點數
                    $get_point = floor($total_red/$web_setting[0]['money2dot']);
                    if($get_point>0){
                        $data = array('create_date'=>time(),'order_date'=>$arr_data[0]['addtime'],'amount'=>$get_point,'cust_id'=>$arr_data[0]['cust_id'],'source'=>'訂單取消 - 扣回紅利點數','type'=>'less','seccode'=>$arr_data[0]['seccode'],'use_day'=>0);
                        $db->insert('point',$data);
                    }
                    //更新訂單點數沒送過
                    $data = array('red_sent'=>0,'isset'=>0);
                    $db->update('orders',$data,"sn = '$orders_id'");
                }
                if($arr_data[0]['red_use']>0){
                    $data = array(
                        'create_date'=>time(),
                        'order_date'=>$arr_data[0]['addtime'],
                        'amount'=>$arr_data[0]['red_use']*$web_setting[0]['usdot'],
                        'cust_id'=>$arr_data[0]['cust_id'],
                        'source'=>'訂單取消 - 退回使用點數',
                        'type'=>'add',
                        'seccode'=>$arr_data[0]['seccode'],
                        'use_day'=>$web_setting[0]['money2dot_day']
                    );
                    $db->insert('point',$data);
                }                
                ?>            
            ...
            if($state==4 && $arr_data[0]['state']!=4){//訂單完成
                if($arr_data[0]['state']==5){//原為取消訂單
                    ...
                    $db->update('orders',$data,"sn = '$orders_id'");
                    if($arr_data[0]['red_use']>0){
                        $data = array('create_date'=>time(),'order_date'=>$arr_data[0]['addtime'],'amount'=>$arr_data[0]['red_use'],'cust_id'=>$arr_data[0]['cust_id'],'source'=>'商品購買 - 使用紅利點數','type'=>'less','seccode'=>$arr_data[0]['seccode'],'use_day'=>$web_setting[0]['money2dot_day']);
                    }
                    ?>
                    <script>
                    (function(i,s,o,g,r,a,m){i['GoogleAnalyticsObject']=r;i[r]=i[r]||function(){
                ...
                if($arr_data[0]['state']!=4){
                    $data = array('pay_state'=>1,'pay_time'=>time());
                    $db->update('orders',$data,"sn = '$orders_id'");
                    if($arr_data[0]['red_sent']==0){//點數未送過
                        //給點(10天後才啟用)
                        $strSQL = "SELECT * FROM web_setting WHERE id='1'";
                        $web_setting = $db->Execute($strSQL);
                        //商品送的點數
                        ...
                        $get_point = floor($total_red/$web_setting[0]['money2dot']);
                        if($get_point>0){
                            $data = array('create_date'=>time()+864000,'order_date'=>$arr_data[0]['addtime'],'amount'=>$get_point,'cust_id'=>$arr_data[0]['cust_id'],'source'=>'商品購買 - 獲得紅利點數','type'=>'add','seccode'=>$arr_data[0]['seccode'],'use_day'=>$web_setting[0]['money2dot_day']);
                            $db->insert('point',$data);
                        }
                        //更新訂單點數送過
                        $data = array('red_sent'=>1);
                        $db->update('orders',$data,"sn = '$orders_id'");
                    }
                }
            }      
            
            // if($arr_data[0]['payment']=='ATM轉帳'){
                // $data = array('pay_state'=>$pay_state);
        ...
        array_push($data, array(
            ...
            'type' => 'select',
            'name' => 'state',
            'help' => '<font color="red">※訂單完成即送出10天後生效的紅利點數；訂單取消即扣回發放紅利點數及退回使用紅利點數。</font>',
            'value' => $arr_data[0]['state'],
            'options'=>$arr_class
            )
        );
        switch ($arr_data[0]['pay_state']) {
            case 0:
                $pay_state = '未付款';
                break;
            case 1:
        ...
        $strSQL = "SELECT id FROM baby_card WHERE seccode = '".$arr_data[0]['seccode']."'";  
    ...
    default:
        ...
        $strSQL = "SELECT * FROM orders where 1 AND order_type = '1'";
        if ($_GET['output'] == '搜尋'){
            if ($keyword != ''){
            ...
            $strSQL = "SELECT * FROM orders where 1 AND order_type = '1' $andSQL ORDER BY sn DESC";

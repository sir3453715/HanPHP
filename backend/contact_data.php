...
case "update":
    $contact_id = $_GET['id'];
    $strSQL = "SELECT * FROM contact WHERE id = '$contact_id'";
    ...
    if ($arr_data['0']['isread'] == '0') {
        $data = array('isread' => '1');
        $a = $db->update('contact', $data, "id = '$contact_id'");
    }
    if ($_POST['flag'] == 'true') {
        ...
        if ($content_re != '' && $arr_data[0]['issend'] == 0) {
            ...
            try {
                ...
                //Content
                $mail->isHTML(true);
                $mail->Subject = 'mousecake 杏屋乳酪蛋糕 - 問題回覆';
                $mail->Body = $output;
    ...
    array_push($data, array(
        'title' => '連絡電話',
        'type' => 'text_show',
        'value' => trim(Encryption::ZxingCrypt($arr_data[0]['tel'], "DECODE")),
        'name' => 'tel',
    )
    );
    array_push($data, array(
    ...
    array_push($data, array(
        ...
        'type' => 'textarea_show',
        'name' => 'content',
        'value' => nl2br($arr_data[0]['content']),
        "required" => true,
    )
    );
    array_push($data, array(
...
case "delete":
    ...
    $db->delete('contact', "id='$contact_id'");
...
default:
    $arr_data = array();
    $andSQL = '';
    $strSQL = "SELECT * FROM contact WHERE 1 $andSQL ORDER BY create_date desc";
    // echo $strSQL;
    // exit;

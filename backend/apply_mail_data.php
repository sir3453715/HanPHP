...
switch ($_GET['func']):
    case "update":
        $apply_mail_id = $_GET['id'];
        $strSQL = "SELECT * FROM apply_mail WHERE id = '$apply_mail_id'";        
        ...
        if($_POST['flag'] == 'true'){
            ...
            // print_r($data);
            // exit;
            $a = $db->update('apply_mail',$data,"id = '$apply_mail_id'");
            if(!$a){
    ...
    case "delete":
        ...
        $db->delete('apply_mail',"id='$apply_mail_id'");
        Func::go_to($func_page . '.php');
        exit;
        break;
    case "insert":
    ...
    default:
        $arr_data = array();
        $andSQL = '';
        $strSQL = "SELECT * FROM apply_mail WHERE 1 $andSQL";
        // echo $strSQL;
        // exit;

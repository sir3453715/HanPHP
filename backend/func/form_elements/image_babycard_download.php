...
if ($vv['value'] != '') {
    ...
    $data = getimagesize($vv['pre_folder'] . '/' . $vv['value']);
    //print_r($size);
    //exit;
    if ($size > 300000) {
        $q = 0.5;
 ...
 */
    ...
    echo '<img src="' . $vv['pre_folder'] . '/' . $vv['value'] . '" ' . $tmp . ' class="img-thumbnail" />';
    echo '<label>
			<a href="' . $vv['pre_folder'] . '/' . $vv['value'] . '" target="_blank" ><button type="button" id="download" name="download" class="btn btn-primary">下載</button> </a>
 		</label>';

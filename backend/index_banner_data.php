...
switch ($_GET['func']):
    case "update":
        $index_banner_id = $_GET['id'];
        $strSQL = "SELECT * FROM index_banner WHERE id = '$index_banner_id'";        
        ...
        if($_POST['flag'] == 'true'){
            ...
            if($_FILES['img']['tmp_name']){
                ...
                if($ext=='png' || $ext=='gif'){
                    Func::ImageResize_Transparent($_FILES['img']['tmp_name'], __SERVER_IMAGES_FOLDER . DIRECTORY_SEPARATOR . $img, 1920, 881);
                }else{
                    Func::ImageResize($_FILES['img']['tmp_name'], __SERVER_IMAGES_FOLDER . DIRECTORY_SEPARATOR . $img, 1920, 881);
                }
            }
            if ($del_img == 'true')
            {
            ...
            $a = $db->update('index_banner',$data,"id = '$index_banner_id'");
            if(!$a){
        ...
        array_push($data, array(
                ...
                'help' => '建議寬高: 1920 * 881 ，或等比例之圖片。其檔案大小不得超過2M',
                'delete' => true
            )
        );
        array_push($data, array(
    ...
    case "delete":
        ...
        $db->delete('index_banner',"id='$index_banner_id'");
        Func::go_to($func_page . '.php');
        exit;
        break;
    ...
    default:
        ...
        $strSQL = "SELECT * FROM index_banner WHERE 1 $andSQL ORDER BY sort desc";
        // echo $strSQL;
        // exit;

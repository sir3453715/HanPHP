...
switch ($_GET['func']):
    case "update":
        $contact_mail_id = $_GET['id'];
        $strSQL = "SELECT * FROM contact_mail WHERE id = '$contact_mail_id'";        
        ...
        if($_POST['flag'] == 'true'){
            ...
            // print_r($data);
            // exit;
            $a = $db->update('contact_mail',$data,"id = '$contact_mail_id'");
            if(!$a){
    ...
    case "delete":
        ...
        $db->delete('contact_mail',"id='$contact_mail_id'");
        Func::go_to($func_page . '.php');
        exit;
        break;
    case "insert":
    ...
    default:
        $arr_data = array();
        $andSQL = '';
        $strSQL = "SELECT * FROM contact_mail WHERE 1 $andSQL";
        // echo $strSQL;
        // exit;
